...
async function gracefulShutdown(signal: string) {
  ...
  stopAllDevServers();
  activePtys.forEach((p) => { try { p.kill(); } catch {} });
...
app.use((err: any, _req: any, res: any, _next: any) => {
  console.error("[server] Express route error:", err?.message || err);
  if (!res.headersSent) {
...
// Auto-recover if port is already in use
server.on("error", (err: any) => {
  if (err.code === "EADDRINUSE") {
    console.error(`[agent-server] Port ${PORT} in use — attempting to free it...`);
    try {
      const { execSync } = require("child_process");
      if (process.platform === "win32") {
        execSync(`npx kill-port ${PORT}`, { timeout: 5000, stdio: "pipe" });
      } else {
        execSync(`lsof -ti:${PORT} | xargs kill -9`, { timeout: 5000, stdio: "pipe" });
      }
      console.log(`[agent-server] Port freed — retrying in 1s...`);
      setTimeout(() => app.listen(PORT, () => console.log(`[agent-server] Running on http://localhost:${PORT}`)), 1000);
    } catch {
      console.error(`[agent-server] Could not free port ${PORT}. Run: npx kill-port ${PORT}`);
    }
  }
});
