...
module.exports = function register(ipcMain, ctx, deps = {}) {
  ...
  function buildSystemPrompt(mission, ghInfo = {}) {
    ...
    return [
      ...
      ``,
      `  Format requirements (identical for all surfaces):`,
      `  • Blank line before the trailer.`,
      `  • Trailer alone on its own line, no leading spaces.`,
      `  • Exact spelling and casing as shown above.`,
      `  • One trailer per piece of text — don't stack multiple.`,
      `  • No emoji, no extra signature, no "PR opened by …" line.`,
      ``,
      `  Surfaces that REQUIRE the trailer:`,
      `  • \`git commit -m\` messages → final line.`,
      `  • \`mcp__github__create_pull_request\` body and \`gh pr create --body\` text → final line.`,
      `  • \`mcp__github__add_issue_comment\` / \`mcp__github__create_pull_request_review\` / \`gh pr review --body\` / curl-issued issue + PR comments → final line of the body.`,
      `  • \`mcp__github__create_issue\` body → final line.`,
      ``,
      `  Surfaces that MUST NOT carry the trailer:`,
      `  • The mission's terse summary line ("Done: ..." / "Skipped: ...") shown in the IDE — that's UI text, not a GitHub artefact.`,
      `  • Code you write into source files (the trailer belongs in the COMMIT, not in the code).`,
      `  • Markdown docs you create unless they're a PR body / issue body.`,
      ``,
      `  Quick check: if the text is being posted to github.com on the user's behalf, attach the trailer. If it's a status line shown inside PiPilot, do not.`,
